...
- `RHBM_GEM_ENABLE_UMAP` defaults to `ON`; set it to `OFF` to omit `umap_embedding` and avoid resolving the umappp dependency stack.
- The build option `RHBM_GEM_DEP_PROVIDER=SYSTEM` requires system packages for Eigen3, CLI11, SQLite3, and Boost. It also requires `pybind11` plus a Python 3 interpreter/development module when `BUILD_PYTHON_BINDINGS=ON` and GoogleTest when `BUILD_TESTING=ON`. When UMAP is enabled, installed UMAP packages are preferred and each missing UMAP component uses its pinned FetchContent source; this fallback does not apply to Eigen or any other core dependency.
...
- Any fetched umappp dependency subtree is used to compile the optional `umap_embedding` command, but remains build-only and excluded from the RHBM-GEM install/export flow.

## Components

| Component | Usage in this project | License | Source mode | Upstream license file(s) |
|---|---|---|---|---|
| aarand | Random distributions used transitively by umappp/irlba | MIT | Used only with `RHBM_GEM_ENABLE_UMAP=ON`: installed package preferred in `SYSTEM`, otherwise pinned v1.1.0 fallback; `FETCH` uses pinned v1.1.0 | `https://github.com/LTLA/aarand/blob/v1.1.0/LICENSE` |
...
| irlba | Truncated singular value decomposition used transitively by umappp | MIT | Used only with `RHBM_GEM_ENABLE_UMAP=ON`: installed package preferred in `SYSTEM`, otherwise pinned v3.1.0 fallback; `FETCH` uses pinned v3.1.0 | `https://github.com/libscran/irlba/blob/v3.1.0/LICENSE` |
| knncolle | Euclidean VP-tree nearest-neighbor search used by umappp for `umap_embedding` | MIT | Used only with `RHBM_GEM_ENABLE_UMAP=ON`: installed package preferred in `SYSTEM`, otherwise pinned v3.1.0 fallback; `FETCH` uses pinned v3.1.0 | `https://github.com/knncolle/knncolle/blob/v3.1.0/LICENSE` |
| sanisizer | Integer-size validation used transitively by umappp | MIT | Used only with `RHBM_GEM_ENABLE_UMAP=ON`: installed package preferred in `SYSTEM`, otherwise pinned v0.2.0 fallback; `FETCH` uses pinned v0.2.0 | `https://github.com/LTLA/sanisizer/blob/v0.2.0/LICENSE` |
| subpar | Parallelization abstraction used transitively by umappp | MIT | Used only with `RHBM_GEM_ENABLE_UMAP=ON`: installed package preferred in `SYSTEM`, otherwise pinned v0.5.0 fallback; `FETCH` uses pinned v0.5.0 | `https://github.com/LTLA/subpar/blob/v0.5.0/LICENSE` |
| umappp | Header-only native C++ implementation used by the `umap_embedding` command | BSD-2-Clause | Used only with `RHBM_GEM_ENABLE_UMAP=ON`: installed v3.3.2 preferred in `SYSTEM`, otherwise pinned v3.3.2 fallback; `FETCH` uses pinned v3.3.2 | `https://github.com/libscran/umappp/blob/v3.3.2/LICENSE` |
