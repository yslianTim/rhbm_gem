...
When ROOT support is available, the command also writes a PDF scatter plot beside the CSV with the same stem. Categories `C`, `CA`, `N`, and `O` retain their existing colors, while residue `HOH` with spot `O` is separated from the general O category and plotted as the orange `HOH` category. With the default internal setting `kDrawOtherUmapSpotGraph = false`, unmatched rows are omitted from the plot and its axis ranges, so the plotted point count can be smaller than the CSV data-row count. Developers can set this switch to `true` and rebuild to group unmatched rows under the gray `Other` category; every embedded row is then plotted exactly once. Categories without data are omitted from the legend. When input spot filtering is enabled, every retained row belongs to one of the five configured categories, so the PDF and CSV data-row counts match regardless of the Other drawing switch. Builds without ROOT skip the PDF and report a warning.
