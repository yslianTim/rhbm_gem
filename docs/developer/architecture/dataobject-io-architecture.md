...
Gaussian statistical value types such as `GaussianEstimate` and `GaussianPosterior` are public value objects under `<rhbm_gem/data/object/GaussianStatistics.hpp>`, while analysis-owned containers that store or derive those values remain internal.
