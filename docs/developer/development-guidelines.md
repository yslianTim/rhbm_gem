...
- `[Required]` Keep the `rhbm_gem::command` registry, runtime registration, Python bindings, and public docs aligned.
...
- `[Required]` Keep repository guard checks passing through `lint_repo` (structure, hygiene, logging style, fixture tracking, command-registry sync, and absolute-path checks).
