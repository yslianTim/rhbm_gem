...
$executable potential_analysis -d $database -m $map_name -a $model_name -k $key_tag -j $jobs --training-alpha $alpha_train

# Run Gaus Estimation Result Dumping
echo "Output gaussian estimation results of following model: $key_tag"
$executable result_dump -o $work_dir -d $database -k $key_tag -p 2
