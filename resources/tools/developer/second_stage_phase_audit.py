#!/usr/bin/env python3
"""Parse observation-only phase audit schema 2; never compare across domains."""
import argparse
import json
import re
...
def report(events, counters, summaries):
    ...
    lines = ['# Second-stage phase audit', '',
        ...
        'Facts: candidate objectives and isolated T(x) − x. Observations do not add acceptance decisions.',
    ...
    for summary in sorted(summaries, key=lambda s: s['attempt']):
    ...
    lines += ['## Interpretation limits', '',
        'Local improvements followed by a worse assembly support investigating coupling between clusters. A rejected globally improving correction warrants inspecting its recorded rejection reason; it does not justify relaxing acceptance gates.',
