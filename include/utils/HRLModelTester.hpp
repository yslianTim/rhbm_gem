...
class HRLModelTester
{
    ...
    std::vector<std::vector<std::tuple<float, float>>> m_sampling_entries_list;
    std::vector<std::vector<Eigen::VectorXd>> m_data_array;
...
public:
    ...
    explicit HRLModelTester(int model_par_size, int linear_basis_size);
    explicit HRLModelTester(int model_par_size, int linear_basis_size, int member_size);
    ~HRLModelTester() = default;

    void SetReplicaSize(int replica_size) { m_replica_size = replica_size; }
    ...
    bool RunBetaEstimateTest(double alpha_r);
...
private:
    void BuildDataArray(int replica_size);
    std::vector<std::tuple<float, float>> BuildRandomSamplingEntry(
    ...
    Eigen::VectorXd CalculateMoment(const std::vector<std::tuple<float, float>> & sampling_entries) const;
