...
class AtomicChargeEntry
{
    ...
    std::tuple<double, double> m_model_estimate_ols;
    std::tuple<double, double> m_model_estimate_mdpde;
    std::unordered_map<std::string, std::tuple<double, double>> m_model_estimate_posterior_map;
    std::unordered_map<std::string, std::tuple<double, double>> m_model_variance_posterior_map;
...
public:
    ...
    void AddModelEstimateOLS(double v0, double v1);
    void AddModelEstimateMDPDE(double v0, double v1);
    void AddModelEstimatePosterior(const std::string & key, double v0, double v1);
    void AddModelVariancePosterior(const std::string & key, double v0, double v1);
