...
class MapObject
{
    ...
    size_t m_voxel_size;
    double m_map_value_mean, m_map_value_min, m_map_value_max, m_map_value_sd;
    std::array<int, 3> m_grid_size;
    std::array<double, 3> m_grid_spacing, m_origin, m_map_length;
    std::array<double, 3> m_overflow, m_underflow, m_upper_bound, m_lower_bound;
    std::unique_ptr<double[]> m_map_value_array;
...
public:
    ...
    std::array<double, 3> GetGridSpacing() const { return m_grid_spacing; }
    std::array<double, 3> GetOrigin() const { return m_origin; }
    std::size_t GetMapValueArraySize() const { return m_voxel_size; }
    const double * GetMapValueArray() const { return m_map_value_array.get(); }
    std::array<int, 3> GetIndexFromPosition(const std::array<double, 3> & position) const;
    std::array<int, 3> GetGridIndex(size_t global_index) const;
    std::array<double, 3> GetGridPosition(size_t global_index) const;
    double GetMapValue(size_t global_index) const;
    double GetMapValue(int index_x, int index_y, int index_z) const;
    double GetMapValueMean() const { return m_map_value_mean; }
    double GetMapValueMin() const { return m_map_value_min; }
    double GetMapValueMax() const { return m_map_value_max; }
...
private:
    ...
    void CheckPosition(const std::array<double, 3> & position) const;
