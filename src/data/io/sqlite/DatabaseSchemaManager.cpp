...
constexpr std::string_view kMapInsertSql = R"sql(
...
void InsertLegacyMapRowsIntoFinalV2(
    ...
    database.Prepare(std::string(kMapInsertSql));
    rhbm_gem::SQLiteWrapper::StatementGuard guard(database);
    for (const auto& row : legacy_map_rows) {
...
void MigrateLegacyV1ToFinalV2(rhbm_gem::SQLiteWrapper& database) {
    ...
    DropTableIfExists(database, std::string(kMapTableName));
...
DatabaseSchemaManager::DatabaseSchemaManager(SQLiteWrapper* database) : m_database{database} {
