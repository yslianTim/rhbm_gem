...
constexpr std::string_view kCreateTableSql = R"sql(
...
constexpr std::string_view kInsertSql = R"sql(
...
constexpr std::string_view kSelectSql = R"sql(
    SELECT
        key_tag,
        grid_size_x, grid_size_y, grid_size_z,
        grid_spacing_x, grid_spacing_y, grid_spacing_z,
...
void MapObjectStorage::CreateTables(SQLiteWrapper& database) {
    database.Execute(std::string(kCreateTableSql));
}

void MapObjectStorage::Save(const MapObject& map_object, const std::string& key_tag) {
    m_database->Prepare(std::string(kInsertSql));
...
std::unique_ptr<MapObject> MapObjectStorage::Load(const std::string& key_tag) {
    m_database->Prepare(std::string(kSelectSql));
    SQLiteWrapper::StatementGuard guard(*m_database);
    m_database->Bind<std::string>(1, key_tag);
