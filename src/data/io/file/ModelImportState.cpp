...
ModelImportState::ModelImportState() : m_component_key_system{std::make_unique<ComponentKeySystem>()},
...
void ModelImportState::AddAtomObject(
    int model_number,
...
void ModelImportState::AddBondObject(
...
std::unique_ptr<ModelObject> ModelImportState::TakeModelObject(int preferred_model_number) {
    ...
    for (auto& bond : bond_list) {
        ...
        if (selected_atom_set.find(atom_1) == selected_atom_set.end())
            continue;
        if (selected_atom_set.find(atom_2) == selected_atom_set.end())
            continue;
...
const std::string& ModelImportState::GetEmdID() const {
    return m_map_id;
}

double ModelImportState::GetResolution() const {
    double resolution_value;
    try {
        resolution_value = std::stod(m_resolution);
    } catch (const std::exception& e) {
        Logger::Log(LogLevel::Error,
                    "ModelImportState::GetResolution()" + std::string(e.what()));
        Logger::Log(LogLevel::Error,
                    "Invalid resolution value: " + m_resolution + ". Setting resolution to -1.0");
        resolution_value = -1.0;
    }
    return resolution_value;
}

const std::string& ModelImportState::GetResolutionMethod() const {
    return m_resolution_method;
}

const std::vector<Element>& ModelImportState::GetElementTypeList() const {
...
ModelImportState::GetChainIDListMap() const {
    return m_chain_id_list_map;
}

const std::unordered_map<int, std::vector<std::unique_ptr<AtomObject>>>&
ModelImportState::GetAtomObjectMap() const {
    return m_atom_object_list_map;
}

const std::vector<std::unique_ptr<BondObject>>& ModelImportState::GetBondObjectList() const {
    return m_bond_object_list;
}

std::vector<int> ModelImportState::GetModelNumberList() const {
...
bool ModelImportState::HasModelNumber(int model_number) const {
    return m_atom_object_list_map.find(model_number) != m_atom_object_list_map.end();
}

AtomObject* ModelImportState::GetAtomObjectPtrInTuple(
    int model_number,
    ...
    if (m_atom_object_in_tuple_map.at(model_number).find(atom_tuple_key) ==
        ...
        Logger::Log(LogLevel::Warning,
                    "Atom object (" + chain_id + ", " + comp_id + ", " + seq_id + ", " + atom_id + ") not found in atom object map.");
        return nullptr;
    }
    return m_atom_object_in_tuple_map.at(model_number).at(atom_tuple_key);
}

AtomObject* ModelImportState::GetAtomObjectPtrInAnyModel(
    const std::string& chain_id,
    const std::string& comp_id,
    const std::string& seq_id,
    ...
    for (const auto current_model_number : model_number_list) {
        ...
        if (model_iter == m_atom_object_in_tuple_map.end())
            continue;
        auto atom_iter{model_iter->second.find(atom_tuple_key)};
        if (atom_iter == model_iter->second.end())
            continue;
...
ChemicalComponentEntry* ModelImportState::GetChemicalComponentEntryPtr(ComponentKey key) const {
...
bool ModelImportState::HasComponentBondEntry(
    ComponentKey comp_key, BondKey bond_key) const {
    if (HasChemicalComponentEntry(comp_key) == false) {
        return false;
    }
