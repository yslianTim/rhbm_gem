    ...
    const std::string & source_name)
{
    ...
    {
        LoadHeader(stream);
        if (m_header.symmetry_table_size < 0)
        {
            throw std::runtime_error("CCP4 symmetry table size cannot be negative.");
        }
        ...
        const auto data_offset{
            ...
            + static_cast<std::streamoff>(m_header.symmetry_table_size)
        };
    ...
    catch (const std::exception & ex)
    {
        throw std::runtime_error(
            "CCP4Format::Read() failed for '" + source_name + "': " + ex.what());
    }
}
...
void CCP4Format::LoadHeader(std::istream & stream)
{
    ...
    if (!stream)
    {
        throw std::runtime_error("Failed to read CCP4 header.");
    }
}
...
void CCP4Format::SaveHeader(std::ostream & stream)
{
    ...
    if (!stream)
    {
        throw std::runtime_error("Failed to write CCP4 header.");
    }
}
