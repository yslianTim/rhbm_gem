...
PdbFormat::PdbFormat() : m_data_block{std::make_unique<ModelImportState>()} {
}
...
std::unique_ptr<ModelObject> PdbFormat::ReadModel(
    ...
    m_data_block = std::make_unique<ModelImportState>();
    if (!stream) {
...
void PdbFormat::ScanAtomEntry(char* line, bool is_special, int model_number) {
    ...
    atom_object->SetElement(ChemicalDataHelper::GetElementFromString(element_name));
    ...
    atom_object->SetResidue(
        ChemicalDataHelper::GetResidueFromString(
            string_helper::ConvertCharArrayToString(atom->residue_name), false));
...
void PdbFormat::WriteModel(const ModelObject& model_object, std::ostream& stream, int par) {
    ...
    for (const auto& atom_ptr : model_object.GetAtomList()) {
        ...
        if (element_symbol.size() > 2)
            element_symbol = element_symbol.substr(0, 2);
        ...
        if (AtomLocalPotentialView::For(*atom).IsAvailable() && AtomLocalPotentialView::For(*atom).HasFinalModel(FittingStage::Second))
        {
            b_factor = AtomLocalPotentialView::For(*atom)
                .GetFinalModel(FittingStage::Second)
