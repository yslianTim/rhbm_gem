    ...
    const std::string & name)
{
    ...
    for (auto & [chain_id, gaus_graph] : gaus_graph_ori_map[0])
    {
        for (size_t k = 0; k < main_chain_element_count; k++)
        {
            if (gaus_graph_ori_map[k].find(chain_id) == gaus_graph_ori_map[k].end()
                || gaus_graph_com_map[k].find(chain_id) == gaus_graph_com_map[k].end()
                || gaus_graph_new_map[k].find(chain_id) == gaus_graph_new_map[k].end())
            {
                continue;
            }
            ...
            auto y_range{
                array_helper::ComputeScalingPercentileRangeTuple(y_array, 0.4)
            };
            ...
            legend->SetMargin(0.15f);
            legend->SetNColumns(2);
            legend->AddEntry(gaus_graph_ori_map[k].at(chain_id).get(), "Original", "lp");
            legend->AddEntry(gaus_graph_new_map[k].at(chain_id).get(), "Using Gaussian parameters from this estimation", "lp");
            legend->Draw();
            root_helper::PrintCanvasPad(canvas.get(), file_path);
        }
    }
