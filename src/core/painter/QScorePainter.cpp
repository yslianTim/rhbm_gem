...
private:
    ...
    void PaintAverageQScoreToSequenceSummary(
...
void QScorePainter::Run()
{
    ...
    for (auto * model_object : m_model_object_list)
    {
        ...
        PaintAverageQScoreToSequenceSummary(
            model_object,
            "average_qscore_to_sequence_summary_" + label);
    }
}
    ...
    const std::string & name)
{
    ...
    std::unordered_map<std::string, std::unique_ptr<TGraphErrors>>
        qscore_graph_map[method_count];
    qscore_graph_map[0] =
        plot_builder->CreateAverageQScoreToSequenceIDGraphMap(false, true, false);
    qscore_graph_map[1] =
        plot_builder->CreateAverageQScoreToSequenceIDGraphMap(false, false, true);
    qscore_graph_map[2] =
        plot_builder->CreateAverageQScoreToSequenceIDGraphMap(true, false, true);
    ...
    for (auto & [chain_id, gaus_graph] : qscore_graph_map[0])
    {
        ...
        if (frame == nullptr)
        {
            frame = root_helper::CreateHist2D(
                "frame","", 500, 0.0, 1.0, 500, 0.0, 1.0);
            ...
            root_helper::SetAxisTickAttribute(
                frame->GetXaxis(),
        ...
        legend->AddEntry(
            qscore_graph_map[0].at(chain_id).get(),
            "Original",
            "lp");
        legend->AddEntry(
            qscore_graph_map[1].at(chain_id).get(),
            "Using Gaussian parameters from this estimation",
            "lp");
        legend->Draw();
