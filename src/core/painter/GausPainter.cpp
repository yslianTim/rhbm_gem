...
void GausPainter::PaintAverageQScoreToSequenceSummary(ModelObject * model_object, const std::string & name)
{
    auto file_path{ m_folder_path + name };
    ...
    qscore_graph_map[0] = plot_builder->CreateAverageQScoreToSequenceIDGraphMap(0, true, false);
    qscore_graph_map[1] = plot_builder->CreateAverageQScoreToSequenceIDGraphMap(0, false, true);
    qscore_graph_map[2] = plot_builder->CreateAverageQScoreToSequenceIDGraphMap(1, false, true);
    ...
    for (auto & [chain_id, gaus_graph] : qscore_graph_map[0])
    {
        ...
        short color_list[method_count] = { kRed, kGreen, kBlue };
        for (size_t k = 0; k < method_count; k++)
        {
            root_helper::SetMarkerAttribute(qscore_graph_map[k].at(chain_id).get(), marker_list[k], 1.2f, color_list[k]);
            root_helper::SetLineAttribute(qscore_graph_map[k].at(chain_id).get(), 1, 1, color_list[k]);
        ...
        legend->AddEntry(qscore_graph_map[0].at(chain_id).get(),
            "Original", "lp");
        legend->AddEntry(qscore_graph_map[1].at(chain_id).get(),
