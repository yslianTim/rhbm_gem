...
void ComparisonPainter::PaintGroupGausEstimateComparison(const std::string & name)
{
    ...
    auto canvas{ root_helper::CreateCanvas("test","", 2000, 1600) };
    root_helper::SetCanvasDefaultStyle(canvas.get());
    root_helper::SetCanvasPartition(canvas.get(), col_size, row_size, 0.20f, 0.30f, 0.10f, 0.06f, 0.02f, 0.02f);
    ...
    pad_extra[0] = root_helper::CreatePad("pad_0","", 0.70, 0.00, 1.00, 0.35); // Right bottom
    pad_extra[1] = root_helper::CreatePad("pad_1","", 0.70, 0.35, 1.00, 0.70); // Right middle
    pad_extra[2] = root_helper::CreatePad("pad_2","", 0.70, 0.70, 1.00, 1.00); // Right top
    pad_extra[3] = root_helper::CreatePad("pad_3","", 0.20, 0.00, 0.70, 0.06); // Left bottom
    pad_extra[4] = root_helper::CreatePad("pad_4","", 0.13, 0.12, 0.17, 0.94); // Left middle
    ...
    for (size_t i = 0; i < col_size; i++)
    {
        ...
        {
            data_graph[i][j] = root_helper::CreateGraphErrors();
            sim_with_charge_graph[i][j] = root_helper::CreateGraphErrors();
    ...
    for (size_t i = 0; i < col_size; i++)
    {
        for (size_t j = 0; j < row_size; j++)
        {
            root_helper::FindPadInCanvasPartition(canvas.get(), static_cast<int>(i), static_cast<int>(j));
            root_helper::SetPadLayout(gPad, 1, 1, 0, 0, 0, 0);
            root_helper::SetPadFrameAttribute(gPad, 0, 0, 0, 0, 4000, 0);
            ...
            frame[i][j] = root_helper::CreateHist2D(Form("hist_%d_%d", static_cast<int>(i), static_cast<int>(j)),"", 500, x_min[i], x_max[i], 500, y_min[j], y_max[j]);
            ...
            root_helper::SetAxisTickAttribute(frame[i][j]->GetXaxis(), static_cast<float>(y_factor*0.08/x_factor), 504);
            root_helper::SetAxisTitleAttribute(frame[i][j]->GetYaxis(), 0.0f);
            root_helper::SetAxisLabelAttribute(frame[i][j]->GetYaxis(), 50.0f, 0.02f, 133);
            ...
            auto element_label_x{ painter_internal::GetMainChainElementLabel(i) };
            auto ref_element_label_x{ painter_internal::GetMainChainElementLabel(ref_id[i]) };
            auto element_color_x{  painter_internal::GetMainChainElementColor(i)};
            auto element_label_y{ painter_internal::GetMainChainElementLabel(j) };
            auto ref_element_label_y{ painter_internal::GetMainChainElementLabel(3) };
            frame[i][j]->GetXaxis()->SetTitle("");
            frame[i][j]->GetYaxis()->SetTitle("");
            frame[i][j]->Draw();

            auto element_marker{ painter_internal::GetMainChainElementSolidMarker(i) };
            root_helper::SetMarkerAttribute(data_graph[i][j].get(), element_marker, 2.0f, element_color_x);
            root_helper::SetLineAttribute(sim_with_charge_graph[i][j].get(), 1, 3, kRed);
            root_helper::SetLineAttribute(sim_no_charge_graph[i][j].get(),   2, 3, kGray+2);
            root_helper::SetLineAttribute(sim_additional_graph[i][j].get(),  3, 3, kBlue);
            ...
            if (i == 0)
            {
                info_text[j] = root_helper::CreatePaveText(-1.28, 0.25, -0.50, 0.75, "nbNDC ARC", true);
                root_helper::SetPaveTextDefaultStyle(info_text[j].get());
                root_helper::SetPaveAttribute(info_text[j].get(), 0, 0.1);
                root_helper::SetFillAttribute(info_text[j].get(), 1001, kAzure-7);
                root_helper::SetTextAttribute(info_text[j].get(), 70.0f, 133, 22, 0.0, kYellow-10);
            ...
            if (j == row_size - 1)
            {
                info_x_text[i] = root_helper::CreatePaveText( 0.10, 1.01, 0.90, 1.27, "nbNDC ARC", true);
                root_helper::SetPaveTextDefaultStyle(info_x_text[i].get());
                root_helper::SetPaveAttribute(info_x_text[i].get(), 0, 0.2);
                root_helper::SetFillAttribute(info_x_text[i].get(), 1001, element_color_x, 0.50f);
                root_helper::SetTextAttribute(info_x_text[i].get(), 70.0f, 133, 22);
                root_helper::SetLineAttribute(info_x_text[i].get(), 1, 0);
    ...
    {
        root_helper::SetPadDefaultStyle(pad_extra[i].get());
        pad_extra[i]->Draw();
    }
    ...
    const std::string y_axis_title[2]
    {
        Form("#tau_{#color[%d]{#font[102]{%s}}} / #tau_{#color[%d]{#font[102]{%s}}}",
            painter_internal::GetMainChainElementColor(0),
            painter_internal::GetMainChainElementLabel(0).data(),
            painter_internal::GetMainChainElementColor(1),
            painter_internal::GetMainChainElementLabel(1).data()),
    ...
    root_helper::SetPadMarginInCanvas(gPad, 0.08, 0.01, 0.10, 0.01);
    root_helper::SetPadLayout(gPad, 1, 1, 0, 0, 0, 0);
    root_helper::SetPadFrameAttribute(gPad, 0, 0, 0, 0, 4000, 0);
    frame_sim[0] = root_helper::CreateHist2D("hist_sim_0","", 500, x_min_sim, x_max_sim, 500, y_min_sim[0], y_max_sim[0]);
    root_helper::SetAxisTitleAttribute(frame_sim[0]->GetXaxis(), 70.0f, 1.0f, 133);
    root_helper::SetAxisLabelAttribute(frame_sim[0]->GetXaxis(), 50.0f, 0.005f, 133);
    root_helper::SetAxisTickAttribute(frame_sim[0]->GetXaxis(), 0.05f, 506);
    root_helper::SetAxisTitleAttribute(frame_sim[0]->GetYaxis(), 75.0f, 0.9f, 133);
    root_helper::SetAxisLabelAttribute(frame_sim[0]->GetYaxis(), 50.0f, 0.02f, 133);
    root_helper::SetAxisTickAttribute(frame_sim[0]->GetYaxis(), 0.05f, 506);
    root_helper::SetLineAttribute(frame_sim[0].get(), 1, 0);
    ...
    root_helper::SetPadMarginInCanvas(gPad, 0.08, 0.01, 0.01, 0.10);
    root_helper::SetPadLayout(gPad, 1, 1, 0, 0, 0, 0);
    root_helper::SetPadFrameAttribute(gPad, 0, 0, 0, 0, 4000, 0);
    frame_sim[1] = root_helper::CreateHist2D("hist_sim_1","", 500, x_min_sim, x_max_sim, 500, y_min_sim[1], y_max_sim[1]);
    root_helper::SetAxisTitleAttribute(frame_sim[1]->GetXaxis(), 0.0f);
    root_helper::SetAxisLabelAttribute(frame_sim[1]->GetXaxis(), 0.0f);
    root_helper::SetAxisTickAttribute(frame_sim[1]->GetXaxis(), 0.05f, 506);
    root_helper::SetAxisTitleAttribute(frame_sim[1]->GetYaxis(), 75.0f, 0.9f, 133);
    root_helper::SetAxisLabelAttribute(frame_sim[1]->GetYaxis(), 50.0f, 0.02f, 133);
    root_helper::SetAxisTickAttribute(frame_sim[1]->GetYaxis(), 0.05f, 506);
    root_helper::SetLineAttribute(frame_sim[1].get(), 1, 0);
    ...
    frame_sim[1]->Draw();
    auto title_sim_text{ root_helper::CreatePaveText( 0.27, 0.73, 0.95, 0.90, "nbNDC ARC", false) };
    root_helper::SetPaveTextDefaultStyle(title_sim_text.get());
    root_helper::SetPaveAttribute(title_sim_text.get(), 0, 0.2);
    root_helper::SetFillAttribute(title_sim_text.get(), 1001, kAzure-7);
    root_helper::SetTextAttribute(title_sim_text.get(), 70.0f, 133, 22, 0.0, kYellow-10);
    root_helper::SetLineAttribute(title_sim_text.get(), 1, 0);
    ...
    {
        pad_extra[i]->cd();
        root_helper::SetMarkerAttribute(simulation1_graph[i].get(), 20, 1.5f, kRed);
        root_helper::SetLineAttribute(simulation1_graph[i].get(), 1, 3, kRed);
        root_helper::SetMarkerAttribute(simulation2_graph[i].get(), 20, 1.5f, kBlue);
        root_helper::SetLineAttribute(simulation2_graph[i].get(), 2, 3, kBlue);
        root_helper::SetMarkerAttribute(simulation3_graph[i].get(), 20, 1.5f, kGray+2);
        root_helper::SetLineAttribute(simulation3_graph[i].get(), 3, 3, kGray+2);
    ...
    for (int i = 0; i < 2; i++)
    {
        data_text[i] = root_helper::CreatePaveText(x_pos[i], 0.50, x_pos[i+1], 0.97, "nbNDC", false);
        root_helper::SetPaveTextDefaultStyle(data_text[i].get());
        root_helper::SetFillAttribute(data_text[i].get(), 4000);
        root_helper::SetTextAttribute(data_text[i].get(), 60.0f, 133, 12);
    }
    auto count{ 0 };
    ...
    auto legend{ root_helper::CreateLegend(0.02, 0.00, 1.00, 0.45, false) };
    root_helper::SetLegendDefaultStyle(legend.get());
    root_helper::SetTextAttribute(legend.get(), 55.0f, 133, 12);
    ...
    root_helper::SetTextAttribute(bottom_title_text.get(), 80.0f, 133, 22);
    bottom_title_text->AddText("Width #tau_{#font[2]{t_{i}}}");
    bottom_title_text->Draw();

    pad_extra[4]->cd(); // left Y title
    auto left_title_text{ root_helper::CreatePaveText(0.0, 0.0, 1.0, 1.0, "nbNDC", false) };
    root_helper::SetPaveTextDefaultStyle(left_title_text.get());
    root_helper::SetFillAttribute(left_title_text.get(), 4000);
    root_helper::SetTextAttribute(left_title_text.get(), 80.0f, 133, 22);
    left_title_text->AddText(Form("Amplitude Ratio #font[1]{A}_{#font[2]{t_{i}}} / #font[2]{A}_{#color[%d]{#font[102]{%s}}}",
                    painter_internal::GetMainChainElementColor(3), painter_internal::GetMainChainElementLabel(3).data()));
...
void ComparisonPainter::PaintGausEstimateResidueClassDenseComparison(const std::string & name)
{
    ...
    const int row_size{ 4 };
    auto canvas{ root_helper::CreateCanvas("test","", 2000, 1800) };
    root_helper::SetCanvasDefaultStyle(canvas.get());
    root_helper::SetCanvasPartition(canvas.get(), col_size, row_size, 0.09f, 0.00f, 0.09f, 0.15f, 0.01f, 0.01f);
    root_helper::PrintCanvasOpen(canvas.get(), file_path);
    ...
    {
        auto element_marker{ painter_internal::GetMainChainElementSolidMarker(k) };
        ...
        auto ref_element_color{ painter_internal::GetMainChainElementColor(ref_id[k]) };
        auto ref_element_label{ painter_internal::GetMainChainElementLabel(ref_id[k]) };

        std::vector<double> x_array, y_array;
        for (int i = 0; i < standard_residue_size; i++)
        {
            auto residue{ static_cast<Residue>(i+1) };
            auto data_graph{ root_helper::CreateGraphErrors() };
        ...
        for (int i = 0; i < standard_residue_size; i++)
        {
            ...
            root_helper::FindPadInCanvasPartition(canvas.get(), col_id, row_id);
            root_helper::SetPadLayout(gPad, 1, 1, 0, 0, 0, 0);
            root_helper::SetPadFrameAttribute(gPad, 0, 0, 0, 0, 4000, 0);
            auto x_factor{ root_helper::GetPadXfactorInCanvasPartition(canvas.get(), gPad) };
            auto y_factor{ root_helper::GetPadYfactorInCanvasPartition(canvas.get(), gPad) };
            frame[i] = root_helper::CreateHist2D(Form("hist_%d", i),"", 500, x_min, x_max, 500, y_min, y_max);
            root_helper::SetAxisTitleAttribute(frame[i]->GetXaxis(), 0.0f);
            root_helper::SetAxisLabelAttribute(frame[i]->GetXaxis(), 50.0f, 0.005f, 133);
            root_helper::SetAxisTickAttribute(frame[i]->GetXaxis(), static_cast<float>(y_factor*0.05/x_factor), 504);
            root_helper::SetAxisTitleAttribute(frame[i]->GetYaxis(), 0.0f);
            root_helper::SetAxisLabelAttribute(frame[i]->GetYaxis(), 50.0f, 0.02f, 133);
            ...
            root_helper::SetMarkerAttribute(data_graph_map[k].at(residue).get(), element_marker, 2.0f, element_color);
            root_helper::SetLineAttribute(sim_with_charge_graph_map[k].at(residue).get(), 1, 2, kRed);
            root_helper::SetLineAttribute(sim_no_charge_graph_map[k].at(residue).get(), 2, 2, kGray+2);
            root_helper::SetLineAttribute(sim_additional_graph_map[k].at(residue).get(), 3, 2, kBlue);
            ...
            title_text[i] = root_helper::CreatePaveText(x1_pos, 0.80, x2_pos, 1.00, "nbNDC ARC", true);
            root_helper::SetPaveTextDefaultStyle(title_text[i].get());
            root_helper::SetPaveAttribute(title_text[i].get(), 0, 0.2);
            root_helper::SetTextAttribute(title_text[i].get(), 60.0f, 103, 22, 0.0, kYellow-10);
            root_helper::SetFillAttribute(title_text[i].get(), 1001, kAzure-7);
            title_text[i]->AddText(ChemicalDataHelper::GetLabel(residue).data());
            title_text[i]->Draw();
        }
        canvas->cd();
        auto pad_extra_t{ root_helper::CreatePad("pad_extra_t","", 0.09, 0.86, 1.00, 1.00) };
        pad_extra_t->Draw();
        pad_extra_t->cd();
        root_helper::SetPadDefaultStyle(pad_extra_t.get());
        root_helper::SetFillAttribute(pad_extra_t.get(), 4000);

        auto legend{ root_helper::CreateLegend(0.00, 0.00, 0.40, 1.00, false) };
        root_helper::SetLegendDefaultStyle(legend.get());
        root_helper::SetTextAttribute(legend.get(), 55.0f, 133, 12);
        root_helper::SetFillAttribute(legend.get(), 4000);
        ...
        for (int i = 0; i < 3; i++)
        {
            info_text[i] = root_helper::CreatePaveText(x_pos[i], 0.00, x_pos[i+1], 1.00, "nbNDC", false);
            root_helper::SetPaveTextDefaultStyle(info_text[i].get());
            root_helper::SetFillAttribute(info_text[i].get(), 4000);
            root_helper::SetTextAttribute(info_text[i].get(), 55.0f, 133, 12);
        }
        auto count{ 0 };
        for (auto & resolution : m_resolution_list)
        {
        ...
        canvas->cd();
        auto pad_extra_x{ root_helper::CreatePad("pad_extra_x","", 0.10, 0.00, 1.00, 0.05) };
        pad_extra_x->Draw();
        pad_extra_x->cd();
        root_helper::SetPadDefaultStyle(pad_extra_x.get());
        root_helper::SetFillAttribute(pad_extra_x.get(), 4000);
        ...
        canvas->cd();
        auto pad_extra_y{ root_helper::CreatePad("pad_extra_y","", 0.00, 0.10, 0.05, 0.85) };
        pad_extra_y->Draw();
        pad_extra_y->cd();
        root_helper::SetPadDefaultStyle(pad_extra_y.get());
        root_helper::SetFillAttribute(pad_extra_y.get(), 4000);
        ...
        root_helper::SetTextAttribute(y_title_text.get(), 80.0f, 133, 22);
    ...
    const std::vector<ModelObject *> & model_list, bool use_component_average, Residue residue)
{
    ...
    for (auto model_object : model_list)
    {
        ...
        else
        {
            ...
            if (!entry_view.HasAtomGroup(LocalFittingStage::Third, ref_group_key)) continue;
            ...
            ref_y_value = entry_view.GetAtomGroupPrior(
                LocalFittingStage::Third, ref_group_key).GetDisplayParameter(par_id);
            ref_y_error = entry_view.GetAtomGroupPriorWithUncertainty(
                LocalFittingStage::Third, ref_group_key)
                .GetDisplayStandardDeviation(par_id);
        }
    ...
    bool draw_index, Residue residue)
{
    ...
    for (auto model_object : model_list)
    {
        ...
        else
        {
            ...
            ref_y_value = entry_view.GetAtomGroupPrior(
                LocalFittingStage::Third, ref_group_key).GetDisplayParameter(0);
            ref_y_error = entry_view.GetAtomGroupPriorWithUncertainty(
                LocalFittingStage::Third, ref_group_key)
                .GetDisplayStandardDeviation(0);
        }
        ...
        if (draw_index == true)
        {
            ...
            root_helper::SetTextAttribute(latex, 35.0f, 103, 21, 0.0, kAzure-7);
