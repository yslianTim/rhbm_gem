      ...
      m_attempt(attempt), m_domain_id(domain_id)
{
    ...
    Add("baseline", {}, BuildSecondStageModelSnapshot(m_context, baseline).node, {}, 1.0, "baseline", "", true);
}
void PhaseAudit::Add(std::string stage, ClusterKey key, FittedGaussianSnapshot state,
    ...
    bool recertify) noexcept
{
    try
    {
        Add(std::string(stage), key, BuildSecondStageModelSnapshot(m_context, state).node,
            ...
            factor, std::string(disposition), std::string(reason), recertify);
    }
    catch (...) { m_capture_failures++; }
}
void PhaseAudit::CaptureState(std::string_view stage, const FitState & state) noexcept
{
    try
    {
        ...
        Add(std::string(stage), {}, BuildSecondStageModelSnapshot(m_context, state).node,
            std::move(parent), 1.0, "observed", "", true);
...
void PhaseAudit::CaptureOperator(const FixedPointOperatorEvidence & evidence) noexcept
{
    try
    {
        ...
        Add("unrestricted-proposal", {}, complete ? evidence.state : FittedGaussianSnapshot{}, {},
            1.0, complete ? "observed" : "unavailable", complete ? "" : "incomplete-production-operator", true);
    }
    catch (...) { m_capture_failures++; }
}
void PhaseAudit::Missing(std::string_view stage, const ClusterKey & key, std::string_view reason) noexcept
{
    try { Add(std::string(stage), key, {}, {}, 1.0, "unavailable", std::string(reason), false); }
    catch (...) { m_capture_failures++; }
}
...
void PhaseAudit::CaptureSearchAssembly() noexcept
{
    try
    {
        ...
        CaptureState("assembly-before-polish", state);
    }
    catch (...) { m_capture_failures++; }
}
    ...
    const FitState & final_state) noexcept
{
    ...
    try
    {
        ...
        const auto evaluate_operator = [&](const FittedGaussianSnapshot & snapshot)
        {
            OperatorAudit result;
            operator_count++;
            try
        ...
        for (const auto & event : m_events)
        {
            ...
            const std::string line{ "Second-stage phase audit: schema=2, payload={\"attempt\":" + std::to_string(m_attempt) +
    ...
    Logger::Log(LogLevel::Debug, "Second-stage phase audit counters: schema=2, payload={\"attempt\":" +
...
[[maybe_unused]] static std::string_view PhaseAuditRejectionReason(const CandidateDecisionEvidence & diagnostic)
{
    ...
    if (diagnostic.rejected_by_previous) return "previous";
    return "strict-improvement";
}

#ifdef RHBM_GEM_ENABLE_SECOND_STAGE_AUDIT_TRACE
void ObservePhaseMissing(SecondStageObservationSession * observation, std::string_view stage, const ClusterKey & key, std::string_view reason) noexcept
{
    if (observation && observation->phase_audit) observation->phase_audit->Missing(stage, key, reason);
}
void ObservePhaseState(SecondStageObservationSession * observation, std::string_view stage, const FitState & state) noexcept
{
    if (observation && observation->phase_audit) observation->phase_audit->CaptureState(stage, state);
}
void ObservePhaseCandidate(SecondStageObservationSession * observation, std::string_view stage, const ClusterKey & key,
    const FitStateView & state, const FitStateView * parent, double factor, std::string_view disposition,
    std::string_view reason, bool recertify) noexcept
{
    if (observation && observation->phase_audit) observation->phase_audit->Capture(stage, key, state, parent, factor, disposition, reason, recertify);
}
void ObservePhaseLocalPolish(SecondStageObservationSession * observation, const ClusterKey & key,
    const FitStateView & state, const FitStateView & parent, double factor, bool accepted,
    const CandidateDecisionEvidence & diagnostic) noexcept
{
    if (observation && observation->phase_audit) observation->phase_audit->Capture("local-polish", key, state, &parent, factor,
        accepted ? "accepted" : "rejected", accepted ? "" : PhaseAuditRejectionReason(diagnostic));
}
void ObservePhaseSearchAssembly(SecondStageObservationSession * observation, const FitState & state) noexcept
{
    if (!observation || !observation->phase_audit) return;
    observation->phase_audit->CaptureSearchAssembly();
    observation->phase_audit->CaptureState("assembly-after-polish", state);
}
void ObservePhaseProposal(SecondStageObservationSession * observation, const IterationProposalResult & proposal) noexcept
{
    if (!observation || !observation->phase_audit) return;
    observation->phase_audit->CaptureOperator(proposal.fixed_point_operator);
    observation->phase_audit->CaptureState("production-proposal", proposal.proposal_state);
}
void ObservePhaseFinish(SecondStageObservationSession * observation, const FitOptions & options, const std::vector<double> & ridge,
    const SuspiciousBlockActivity & activity, const IterationProposalResult & proposal, const FitState & state) noexcept
{
    if (observation && observation->phase_audit) observation->phase_audit->Finish(options, ridge, activity, proposal, state);
}
#endif
