    ...
    const std::vector<ClusterCandidateDiagnostic> & diagnostic_list)
{
    ...
    for (const auto & cluster_diagnostic : diagnostic_list)
    {
        ...
        message << ", stored-best = ";
        AppendObjectiveBreakdown(message, diagnostic.history ? diagnostic.history->stored_best_objective : std::nullopt);
        message << ", best-reference = ";
        AppendObjectiveBreakdown(message, diagnostic.history ? diagnostic.history->best_objective : std::nullopt);
    ...
    double total_milliseconds)
{
    ...
    message_debug
        << " - full_state_materializations = " << counters.m_full_state_materialization_count.load() << "\n"
        << " - gaussian_cache_hit/miss = "
        << counters.m_gaussian_cache_hit_count.load() << "/" << counters.m_gaussian_cache_miss_count.load() << "\n"
        << " - objective_recomputed/reused_samples = "
        << counters.m_objective_recomputed_sample_count.load() << "/"
        << counters.m_objective_reused_sample_count.load() <<"\n"
        << " - solver_symbolic_analyses = " << symbolic_analysis_count << "\n"
        << " - topology_rebuilds/partition_changes = "
        << counters.m_topology_rebuild_attempt_count << "/" << counters.m_topology_partition_change_count<< "\n"
        << " - boundary_reconciliations/backtracked/rejected = "
        << counters.m_boundary_reconciliation_attempt_count << "/"
        << counters.m_boundary_reconciliation_backtracked_count << "/"
        << counters.m_boundary_reconciliation_rejected_count << "\n"
        << " - boundary_joint_correction_attempts/accepted/fallback = "
        << counters.m_boundary_joint_correction_attempt_count << "/"
        << counters.m_boundary_joint_correction_accepted_count << "/"
        << counters.m_boundary_joint_correction_fallback_count << "\n"
        << " - boundary_rescues/accepted/fallback/rejected = "
        << counters.m_boundary_rescue_attempt_count << "/"
        << counters.m_boundary_rescue_accepted_count << "/"
        << counters.m_boundary_rescue_fallback_count << "/"
        << counters.m_boundary_rescue_rejected_count << "\n"
        << " - boundary_rescue_exclusions_hard/invalid/no-objective = "
        << counters.m_boundary_rescue_hard_failure_exclusion_count << "/"
        << counters.m_boundary_rescue_invalid_proposal_exclusion_count << "/"
        << counters.m_boundary_rescue_objective_unavailable_exclusion_count << "\n"
        << " - dependency_polish_components/attempted/accepted/fallback = "
        << counters.m_dependency_polish_component_count << "/"
        << counters.m_dependency_polish_attempt_count << "/"
        << counters.m_dependency_polish_accepted_count << "/"
        << counters.m_dependency_polish_fallback_count << "\n"
        << " - dependency_polish_atoms/parameters/rounds = "
        << counters.m_dependency_polish_atom_count << "/"
        << counters.m_dependency_polish_parameter_count << "/"
        << counters.m_dependency_polish_round_count << "\n";
    ...
    audit_performance << std::scientific << std::setprecision(17)
        ...
        << ", candidate-ms=" << counters.m_candidate_phase_milliseconds
