...
void ComparisonPainter::Painting(void)
{
    ...
    PaintSimulationGaus("test_simulation_plot.pdf");
    PaintDataSimulationComparison("test_comparison_plot.pdf");
}
...
void ComparisonPainter::PaintSimulationGaus(const std::string & name)
{
    ...
    ROOTHelper::SetCanvasPartition(canvas.get(), column_size, row_size, 0.1, 0.01, 0.15, 0.01, 0.01, 0.005);
    ...
    for (int i = 0; i < primary_element_size; i++)
    {
        ...
        for (int j = 0; j < row_size; j++)
        {
            ...
            ROOTHelper::SetMarkerAttribute(sim_buried_charge_graph[i][j].get(), marker_element[i], 1.3, color_element[i]);
            ROOTHelper::SetMarkerAttribute(sim_no_charge_graph[i][j].get(), marker_element[i], 1.3, color_element[i]);
    ...
    for (int i = 0; i < column_size; i++)
    {
        for (int j = 0; j < row_size; j++)
        {
            ...
            frame[i][j] = ROOTHelper::CreateHist2D(Form("hist_%d_%d", i, j),"", 500, 0.0, 1.0, 500, y_min[j], y_max[j]);
...
void ComparisonPainter::PaintDataSimulationComparison(const std::string & name)
{
    ...
    for (int i = 0; i < column_size; i++)
    {
        for (int j = 0; j < row_size; j++)
        {
            ...
            frame[i][j]->GetYaxis()->SetTitle("Amplitude Ratio");
            frame[i][j]->GetYaxis()->CenterTitle();
            frame[i][j]->SetStats(0);
            frame[i][j]->Draw();
