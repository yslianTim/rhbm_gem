...
{
    auto file_path{ m_folder_path + name };
    std::cout <<"- ModelPainter::PaintAtomWidthScatterPlotSingle"<< std::endl;
    auto class_key{ AtomicInfoHelper::GetResidueClassKey() };
...
{
    auto file_path{ m_folder_path + name };
    std::cout <<"- DemoPainter::PaintGroupWidthScatterPlot"<< std::endl;
    auto residue_class{ AtomicInfoHelper::GetResidueClassKey() };
    ...
    for (int i = 0; i < col_size; i++)
    {
        ...
        for (int j = 0; j < row_size; j++)
        {
            ...
            if (i == 0)
            {
                ...
                ROOTHelper::SetFillAttribute(title_y_text[j].get(), 1001, element_color, 0.5f);
                title_y_text[j]->AddText(element_label.data());
                title_y_text[j]->Draw();
            }
        }
        ...
        title_x_text[i]->AddText(model_list.at(static_cast<size_t>(model_id))->GetPdbID().data());
        title_x_text[i]->AddText(model_list.at(static_cast<size_t>(model_id))->GetEmdID().data());
        title_x_text[i]->Draw();
        ...
        resolution_text[i]->AddText(Form("%.2f #AA", model_list.at(static_cast<size_t>(model_id))->GetResolution()));
