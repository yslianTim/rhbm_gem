...
TEST(ChimeraXHelperTest, WritePointsAutoCmm)
{
    ...
    ASSERT_TRUE(ChimeraXHelper::WritePointsAuto(pts, tmp.string(), 2.0f, RGB{0.1f,0.2f,0.3f}, "test"));
...
TEST(ChimeraXHelperTest, WritePointsAutoXyz)
{
    std::vector<std::array<float,3>> pts{{{1.0f,2.0f,3.0f},{4.0f,5.0f,6.0f}}};
    std::filesystem::path tmp = std::filesystem::temp_directory_path()/"chimera_test.xyz";
    ASSERT_TRUE(ChimeraXHelper::WritePointsAuto(pts, tmp.string()));
    std::ifstream ifs(tmp);
    ASSERT_TRUE(ifs.good());
    std::string content((std::istreambuf_iterator<char>(ifs)), std::istreambuf_iterator<char>());
    std::string expected =
