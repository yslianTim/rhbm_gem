...
// The response already has the fixed neighbors subtracted. No target truth is
// accepted here except the explicitly fixed charge in the two-parameter problem.
boost::json::object Fit(const Design &, const Eigen::VectorXd & response,
    double fixed_charge, bool fit_charge, double cutoff, int maximum_iterations = 128);
void Run(const std::string & manifest, const std::string & map,
    const std::string & captures, const std::string & output);
