...
TEST(
    EstimatorTesterTest,
    RunPotentialFittingWorkflowDefersGroupEstimationUntilThirdStage)
{
    ElectricPotential potential_model;
    potential_model.SetModelChoice(0);
    potential_model.SetBlurringWidth(0.5);
    ...
    for (const auto stage : { FittingStage::First, FittingStage::Second })
    {
        ...
        EXPECT_DOUBLE_EQ(analysis_view.GetAtomAlphaG(stage, group_key), 0.0);
        EXPECT_DOUBLE_EQ(
            analysis_view.GetAtomGroupPrior(stage, group_key).GetAmplitude(),
            0.0);
        EXPECT_DOUBLE_EQ(
            analysis_view.GetAtomGroupPrior(stage, group_key).GetWidth(),
            0.0);
        EXPECT_FALSE(fitted_view.GetGaussianResult(stage).posterior.has_value());
    ...
    const auto third_group_keys{
        analysis_view.CollectAtomGroupKeys(FittingStage::Third)
    };
    ASSERT_EQ(third_group_keys.size(), 1u);
    const auto third_group_key{ third_group_keys.front() };
    EXPECT_EQ(
        analysis_view.GetAtomObjectList(
            FittingStage::Third, third_group_key).size(),
        1u);
    EXPECT_TRUE(std::isfinite(
        analysis_view.GetAtomGroupPrior(
            FittingStage::Third, third_group_key).GetAmplitude()));
    EXPECT_GT(
        analysis_view.GetAtomGroupPrior(
            FittingStage::Third, third_group_key).GetWidth(),
        0.0);
    EXPECT_TRUE(std::isfinite(
        analysis_view.GetAtomAlphaG(
            FittingStage::Third, third_group_key)));
    EXPECT_TRUE(
        fitted_view.GetGaussianResult(FittingStage::Third)
            .posterior.has_value());
    ...
    EXPECT_EQ(
        static_cast<std::size_t>(std::stoull(row.at(4))),
    ...
    EXPECT_NEAR(std::stod(row.at(5)), *expected_signal_peeling_ratio, 0.0051);
    EXPECT_NEAR(std::stod(row.at(6)), *expected_tail_peeling_ratio, 0.0051);
    for (std::size_t column = 5; column < 16; column++)
    {
    ...
    EXPECT_NEAR(std::stod(row.at(7)), first_model.GetAmplitude(), 0.0051);
    EXPECT_NEAR(std::stod(row.at(8)), second_model.GetAmplitude(), 0.0051);
    EXPECT_NEAR(std::stod(row.at(9)), final_model.GetAmplitude(), 0.0051);
    EXPECT_NEAR(std::stod(row.at(10)), first_model.GetWidth(), 0.0051);
    EXPECT_NEAR(std::stod(row.at(11)), second_model.GetWidth(), 0.0051);
    EXPECT_NEAR(std::stod(row.at(12)), final_model.GetWidth(), 0.0051);
    EXPECT_NEAR(std::stod(row.at(13)), first_model.GetOffset(), 0.0051);
    EXPECT_NEAR(std::stod(row.at(14)), second_model.GetOffset(), 0.0051);
    EXPECT_NEAR(std::stod(row.at(15)), final_model.GetOffset(), 0.0051);
    for (std::size_t column = 16; column < row.size(); ++column)
    {
...
TEST(EstimatorTesterTest, LocalFittingResultRanksUseThreeNearestAtomsAcrossAllStages)
{
    ...
    constexpr std::array<std::size_t, 3> rank_column_starts{ 16, 19, 22 };
