...
class JointEvidenceTest(unittest.TestCase):
    def manifest(self, directory, model='single_gaus'):
        ...
        widths = [.5, .45, .4]
        value = {'schema_version': 2, 'source': {'model_sha256': sha256_file(paths['model'])},
                 'output': {'map_sha256': sha256_file(paths['map'])},
    ...
    def test_wrong_width_policy_hash_identity_and_version_are_rejected(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            for mutate in mutations:
                bad = copy.deepcopy(value); mutate(bad); write(paths['manifest'], bad)
